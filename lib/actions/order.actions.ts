...
export async function createPayPalOrder(orderId: string) {
  ...
  try {
    ...
    // Reuse existing PayPal order ID if present (e.g. user retried after "take you back to checkout")
    if (order.paymentResult?.id) {
      return {
        success: true,
        message: 'PayPal order created successfully',
        data: order.paymentResult.id,
      }
    }
    ...
    return {
      success: false,
      message: formatError(err) || 'Could not start PayPal payment. Please try again.',
    }
  }
}
...
export async function approvePayPalOrder(
  ...
  try {
    ...
    if (!captureData || captureData.status !== 'COMPLETED') {
      throw new Error('PayPal payment could not be verified. Please try again.')
    }

    // Match by PayPal order ID so retries / stale modal still work (same order may have different paymentResult.id after retry)
    if (order.paymentResult?.id !== captureData.id) {
      const orderByPayPalId = await Order.findOne({
        'paymentResult.id': data.orderID,
      }).populate('user', 'email')
      if (orderByPayPalId) order = orderByPayPalId
    }

    if (order.paymentResult?.id !== captureData.id) {
      throw new Error(
        'Payment could not be matched to this order. Please contact support if you were charged.'
      )
    }
    ...
    console.error('[approvePayPalOrder]', err)
    const message =
      formatError(err) ||
